...
class NhlPlayoffGamesPage extends Component {
  constructor(props) {
    super(props) 
    this.state = {
      ...
      this.getNHLPlayoffGames = this.getNHLPlayoffGames.bind(this)
  ...
  getAllGames = () => {
    let self = this
      API.getNhlPlayoffGames()
        .then(res => {
          console.log('ALL NHL PLAYOFF GAMES: ', res.data)
          self.setState({
            allGames: res.data
          })
  ...
  getNHLPlayoffGames = () => {
    ...
    $.ajax({
      url: 'https://cors-everywhere.herokuapp.com/http://api.sportradar.us/nhl/trial/v7/en/games/2019/PST/schedule.json?api_key=' + nhlPlayoffKey,
      type: 'GET',
  ...
  getResults = () => {
    ...
    // const nhlKey = '2kuh4yhq78h5rdmf9vrsprgg'
    // const nhlKey2 = '4y7q3vsbv9rdj9kbevdfng4j'
    const nhlPlayoffKey = 'y947xs7vmjgpugdgy59z5c2g'

    // API CALL TO GET EACH NHL GAME RESULT (DELAY 1.5 SECONDS)
    for (let m=0; m<yesterdayGamesIds.length; m++) {
      let k = m
      setTimeout ( 
        function() {
          $.ajax({
            url: 'https://cors-everywhere.herokuapp.com/http://api.sportradar.us/nhl/trial/v7/en/games/' + yesterdayGamesIds[k] + '/boxscore.json?api_key=' + nhlPlayoffKey,
            type: 'GET',
            success: function(data) {
        ...
        return(
            <div id='nhlGamesPage'>
              ...
              <AdminBar />
                ...
                <div id='nhlGames'>
                  <h1 className='adminDatabaseHeader'>NHL GAMES DATABASE</h1>
                  <div className='row adminDatabaseControlsRow'>
                    <Button className='adminDatabaseControlsButton' onClick={this.getNHLPlayoffGames}>Upload Season Schedule</Button>
...
export default NhlPlayoffGamesPage